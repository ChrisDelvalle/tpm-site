...
- [x] Review the old glossary URL in
      `src/content/articles/history/death-of-a-meme-or-how-leo-learned-to-stop-worrying-and-love-the-bear.md`.
- [x] Review the extra `>` after a closing paragraph tag in
      `src/content/articles/history/wittgensteins-most-beloved-quote-was-real-but-its-fake-now.md`.
- [x] Decide whether the remaining remote Discord CDN image references should
      stay remote or be migrated into `src/assets/`. Two available local copies
      were switched to `src/assets/`; the remaining Discord URL returned 404
      and no local copy was found.
- [x] Review WordPress-style paragraph/span markup in
      `src/content/articles/memeculture/newfriends-and-the-generation-gap.md`
      before any cleanup.
- [x] Review legacy caption `<div class="caption">` markup before deciding
      whether to keep raw HTML, create a component, or convert to Markdown.
- [x] Review raw HTML tables before deciding whether conversion to Markdown is
      safe.
- [x] Review iframe embeds only for rendering, accessibility, and layout; raw
      iframe HTML is acceptable when it expresses the intended embed.

## Optional Cleanup, Not Migration Blockers

- [x] Convert mechanically safe raw HTML to Markdown only after explicit
      approval.
- [x] Add or improve image alt text only when the intended description is clear
      or supplied by the user.
- [ ] Convert MDX articles back to Markdown only if they no longer need
      components.
- [x] Migrate reachable remote article images into `src/assets/` only after
      deciding that preserving a local copy is worth the source-content change.

## Manual Verification Protocol

- [x] Inspect the git diff before and after every content-source change.
- [x] Verify no author wording changed unless explicitly requested.
- [x] Run the relevant repository checks after content changes.
...
- [ ] Verify article images, embeds, captions, links, headings, and tables still
      render as intended.
- [x] For MDX changes, verify imports, component props, generated HTML, and
