...
| `build`                           | Runs the raw Astro/Pagefind build, generates same-directory article PDFs, then applies the production-safe generated-output optimizer stack to `dist/`.                                          |
| `build:optimize`                  | Applies the production generated-output optimization stack to an existing `dist/`: Lightning CSS, SVGO, conservative Oxc JavaScript whitespace optimization, and orphan `_astro` raster cleanup. |
...
| `test:site-instance`              | Builds the external fixture site with fixture content/config, generated PDFs, and production optimization. Blocking proof for platform/site split changes.                                       |
