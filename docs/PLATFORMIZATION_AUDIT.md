...
This audit is a planning baseline. The in-repo `site/` migration is now
specified in `docs/SITE_INSTANCE_MIGRATION.md`; remaining platformization work
should compare against the current `site/` directory and the external fixture
proof rather than the older pre-migration source-root coupling described in some
risk rows below.
...
| Content roots        | `src/content.config.ts` now reads resolver-backed site content roots.                                                           | Promote the external fixture proof into a production external-instance build workflow.                                  | High   |
