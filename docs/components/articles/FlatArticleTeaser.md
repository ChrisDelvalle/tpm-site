...
`FlatArticleTeaser` renders one compact article-like link for dense editorial
lists. It is the primitive for homepage rails such as Announcements and Start
Here where a full article card would add too much visual noise.
...
- Reuse the same `ArticleListItem` data shape as full article lists.
...
- It does not fetch content or inspect collections.
