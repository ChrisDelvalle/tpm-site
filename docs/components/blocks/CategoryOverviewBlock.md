...
Source: `src/components/blocks/CategoryOverviewBlock.astro`
...
keep focus-visible states intact for any interactive descendants. Use
`headingLevel={1}` for standalone category pages and `headingLevel={2}` when
this block is embedded inside a page that already owns the `h1`.
